export const name = "kindling.deliverOutreachVariants";
export const description = "Normalise Kindling outreach positioning and three draft variants, then deliver them to the configured WApp webhook.";
...
function buildPayload(input: JsonObject): JsonObject {
  ...
  const recommended = drafts[0] ?? normaliseVariant({}, 1);
  ...
  return {
    ...
    result: {
      ...
      variants: drafts,
      subject: text(recommended.subject, companyName ? `Quick idea for ${companyName}` : "Quick idea"),
      body: text(recommended.body),
