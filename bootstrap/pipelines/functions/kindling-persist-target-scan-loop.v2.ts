...
export const name = "kindling.persistTargetScanLoop.v2";
...
export default async function run(input: JsonObject) {
  ...
  const searchSlices = arrayValue(draft.searchSlices)
    .map((slice) => normaliseStrategy(slice, "searched"))
    .filter((slice) => text(slice.status, "searched") !== "planned");
  const plannedNextStrategies = arrayValue(draft.plannedNextStrategies)
    .map((slice) => normaliseStrategy(slice, "planned"));
  const historySummary = {
    ...
    confidence: confidence(draft.confidence),
    artifactPath: path,
    companyNames: companies.slice(0, 50).map((company) => text(company.name)).filter(Boolean),
  };
  ...
  if (!webhook) {
    return { ...payload, persistence, historySummary };
  }
  const webhookResult = await postJson(webhook, payload);
  return { ...payload, persistence: { ...persistence, webhook: webhookResult }, historySummary };
}
